...
def buscar_datos(valor):
    try:
        ...
        messagebox.showerror("Error al conectar a la base de datos o ejecutar la consulta: ", e)
        return []
    ...
    try:
        for x in os.listdir(folder):
            if x.endswith(extensions):
                ...
                if len(filepath) <= 8:
                    resultados = buscar_datos(filepath)
                    ...
                    if fila_seleccionada:
                        DNI = fila_seleccionada.get('codigo_trabajador')
                        NOMBRE = fila_seleccionada.get('Nombre_Completo')
                        CARGO = fila_seleccionada.get('cargo_descripcion')
                        AREA = fila_seleccionada.get('area_descripcion')
                        GERENCIA = fila_seleccionada.get('centro_costos_descripcion')
                        SEDE = fila_seleccionada.get('descripcion_sede')
                        ACTIVO = fila_seleccionada.get('activo_cesado')

                        nuevo_nombre = f"{DNI} - {NOMBRE.upper()} - {AREA.upper()} - {GERENCIA.upper()} - {fecha_actual} - {SEDE.strip()}.pdf"
                        os.rename(os.path.join(folder, x), os.path.join(folder, nuevo_nombre))
                else:
                    continue
        ...
        for x in os.listdir(folder):
            if x.endswith(extensions):
                ...
                if len(filepath) <= 12:
                    ...
                    columnas = ['Host', 'DNI', 'Colaborador', 'Área', 'Sede2', 'SN']
